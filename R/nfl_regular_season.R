...
if (FALSE) {
  g <- nflreadr::load_schedules(2025)
  s <- nflseedR::nfl_standings(g, ranks = "DIV")
  ns <- nfl_regular_season(s)
  ns_all <- purrr::map(
  ...
  actual_scheds <- nflreadr::load_schedules(2010:2025) |>
    ...
    dplyr::mutate(
      ...
      opp = nflreadr::clean_team_abbrs(opp)
    )

  # take all of nflseedR's calculated team, opp pairs and compare them to
  # actually played games.
  ns_all |>
    # we don't catch older seasons without the 17th game so we need to remove
    # them here
    dplyr::filter_out(season < 2021 & opp_type == "cross_17th") |>
    # this line will remove all rows where the team, opp, location data doesn't match
    dplyr::anti_join(actual_scheds) |>
    # now we join actual scheds neutral site games because those should mismatch
    dplyr::left_join(
      actual_scheds |> dplyr::filter(location == "neutral"),
      by = c("season", "team", "opp"),
      suffix = c("_seb", "_actual")
    ) |>
    # this shows we mismatched 60 neutral site games (yay) and one additional game
    # that's the cancelled 2022 W17 BUF CIN matchup
    dplyr::count(location_seb, location_actual)
}
