...
Let's check out the playoff games from the fourth simulation, where the Bears went 15-1.

```{r}
sims$games %>% dplyr::filter(sim == 4, game_type != "REG") %>% knitr::kable()
```

In this simulation, the Bears after their playoff bye defeated the Buccaneers by 11, then
the Washington Football Team by 17 in the NFC Championship Game, and finally the Chargers
by 13 in the Super Bowl.
...
won the Super Bowl in a nice 69% of seasons! Meanwhile, the teams at the bottom
...
The second simulation has been the worst of these six, while the fourth is the highest.
Let's check out the early Bears games from that fourth season:
...
First, note that only the first three weeks have the result filled in. Again, this because
the `test_week = 3` input aborted the simulation after Week 3, which was useful for seeing
the Elo above. It also makes sense that this would be a simulation where the Bears' Elo is
higher, since they won all 3 games.
...
[Lee Sharpe's NFL game data](https://github.com/leesharpe/nfldata/tree/master/data). The earliest season you can simulate is 2002.
...
your function, is a list of four data frames. Here are the contents of each:
...
* **teams** - One row per team per simulation.
  ...
  * **team** - The abbreviation reperesenting the team
  ...
  * **div_pct** - The win rate of the teams in games played agains teams in the same division.
  * **conf_pct** - The win rate of the teams in games played agains teams in the same conference.
...
* **teams_wins** - Each team has a row for 0 wins, 0.5 wins, 1 win, 1.5 wins, ... , 15.5 wins, 16 wins. These are aggergated across all simulations and are intending to represent your model's probability for teams being below or above a certain win total, making them an excellent reference for using your model to make win total bets.
  * **team** - The abbreviation reperesenting the team
