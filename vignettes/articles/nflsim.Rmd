...
```{r}
sims$teams %>% dplyr::filter(team == "CHI") %>% utils::head(6) %>% select(sim, team, wins, seed, draft_order) %>% knitr::kable()
```
...
the Elo values for teach team and store them in the `teams` data. This column will
...
test$teams %>% filter(team == "CHI") %>% utils::head() %>% knitr::kable()
```
